...
import { BaseCommandInteraction, CacheType, CommandInteraction, Message, TextChannel, User } from "discord.js";
...
export class MusicQueue {
  ...
  private async sendPlayingMessage(newState: any) {
    ...
    try {
      playingMessage = await this.interaction.editReply((newState.resource as AudioResource<Song>).metadata.startMessage()) as Message;
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        case "⏭":
          ...
          await this.bot.commands.get("skip")!.execute(this.interaction);
          break;

        case "⏯":
          reaction.users.remove(user).catch(console.error);
          if (this.player.state.status == AudioPlayerStatus.Playing) {
            await this.bot.commands.get("pause")!.execute(this.interaction);
          } else {
            await this.bot.commands.get("resume")!.execute(this.interaction);
          }
          break;
        ...
        case "🔁":
          reaction.users.remove(user).catch(console.error);
          await this.bot.commands.get("loop")!.execute(this.interaction);
          break;

        case "🔀":
          reaction.users.remove(user).catch(console.error);
          await this.bot.commands.get("shuffle")!.execute(this.interaction);
          break;

        case "⏹":
          reaction.users.remove(user).catch(console.error);
          await this.bot.commands.get("stop")!.execute(this.interaction);
          collector.stop();
          break;
